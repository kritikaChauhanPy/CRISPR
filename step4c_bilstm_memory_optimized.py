"""
BiLSTM-4ch Genetic Algorithm - Memory Optimized for 6GB GPU
"""
...
def create_individual():
    return {
        'hidden_size': random.choice([64, 96, 128]),
        'num_layers': 1,
...
def mutate(individual, mutation_prob=0.2):
    ...
    if random.random() < mutation_prob:
        mutated['hidden_size'] = random.choice([64, 96, 128])
    
    if random.random() < mutation_prob:
        mutated['dropout'] = round(random.uniform(0.1, 0.5), 2)
    
    if random.random() < mutation_prob:
        mutated['learning_rate'] = random.choice([0.0001, 0.0005, 0.001])
...
def run_genetic_algorithm(model_class, model_name, X_train, X_val, y_train, y_val,
    ...
    print(f"Memory optimized: hidden≤128, layers=1, batch=32, FP16")
...
def create_visualizations(results_df, model_name, best_auprc):
    ...
    axes[1, 0].scatter(results_df['learning_rate'], results_df['val_auprc'], alpha=0.6, s=50, color='green')
    axes[1, 0].set_xlabel('Learning Rate', fontweight='bold')
    ...
    axes[1, 1].barh(range(10), top_10['val_auprc'].values, color='purple', alpha=0.7)
    axes[1, 1].set_yticks(range(10))
    axes[1, 1].set_yticklabels([f"Config {i+1}" for i in range(10)])
    axes[1, 1].set_xlabel('Validation AUPRC', fontweight='bold')
    ...
    with open(f'{output_dir}/SUMMARY.txt', 'w') as f:
        ...
        f.write("-"*80 + "\n")
        f.write("Hidden size: max 128 (vs 256 standard)\n")
        f.write("Layers: 1 (vs 2 standard)\n")
        f.write("Batch size: 32 (fixed)\n")
        f.write("Precision: Mixed FP16\n\n")
        ...
        f.write("COMPARISON:\n")
        f.write("-"*80 + "\n")
        f.write(f"Random search: 0.3864\n")
        f.write(f"GA optimized: {best_auprc:.4f}\n")
        improvement = ((best_auprc - 0.3864) / 0.3864) * 100
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("BiLSTM-4ch GENETIC ALGORITHM - MEMORY OPTIMIZED")
    print("="*80)
    ...
    best_config, best_auprc, results_df = run_genetic_algorithm(
        model_class=ConfigurableBiLSTM,
        model_name='BiLSTM-4ch',
        X_train=X_train,
        X_val=X_val,
        y_train=y_train,
        y_val=y_val,
    ...
    results_df.to_csv('results/true_ga/BiLSTM-4ch_progress.csv', index=False)
    
    print(f"\n{'='*80}")
    print("BiLSTM-4ch COMPLETE")
    print(f"{'='*80}\n")
    ...
    benchmarks = [
        ('BiLSTM Random Search', 0.3864),
        ('Rahman LSTM Validation', 0.7427),
        ('BiLSTM GA (mem opt)', best_auprc)
    ]
    
    for name, score in benchmarks:
        marker = ' ✓' if 'GA' in name else ''
    ...
    create_visualizations(results_df, 'BiLSTM-4ch', best_auprc)
    
    print(f"\n{'='*80}")
    print("ALL COMPLETE")
    print(f"{'='*80}")
    print(f"\nResults: results/true_ga/BiLSTM-4ch_progress.csv")
    print(f"Visualizations: results/true_ga/BiLSTM-4ch_visualizations/")
